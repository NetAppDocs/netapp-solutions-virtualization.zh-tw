...
對於 NFS 資料存儲，具有 vSphere Metro Storage Cluster 的ONTAP MetroCluster可確保高可用性。中介器可以避免腦裂情況，現在可以託管在NetApp Console上。
...
您可以使用NetApp Backup and Recovery for VMs 透過SnapMirror和SnapMirror to Cloud 實現 3-2-1 備份策略。您可以從三個位置中的任一個執行復原。

Trident Protect 或NetApp Backup and Recovery for Kubernetes 可保護 VKS 叢集應用程式。
...
您可以使用 VMware Live Site Recovery 或NetApp Disaster Recovery Disaster Recovery來管理虛擬機器的災難復原。ONTAP工具提供 SRA（儲存複製適配器）來協調ONTAP的儲存作業。
...
ONTAP工具為資料儲存區複製設定提供了 UI。NetApp Console也可用於儲存陣列之間的複製。SnapCenter Plug-in for VMware vSphere利用現有的SnapMirror關係式來實作 SnapShot。
...
您可以使用ONTAP工具或NetApp Console來設定複製關係。

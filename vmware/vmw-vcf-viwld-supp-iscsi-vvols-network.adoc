...
summary: 為 VI 工作負載域中的 ESXi 主機上的 iSCSI 儲存配置網路。您將為 iSCSI 流量建立分散式連接埠群組，並使用 vSphere 用戶端設定 VMkernel 適配器，以實現可靠的連線和多路徑。 
---
= 在 VCF VI 工作負載域中的 ESXi 主機上設定 iSCSI 網路
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
為 VI 工作負載域中的 ESXi 主機上的 iSCSI 儲存配置網路。您將為 iSCSI 流量建立分散式連接埠群組，並使用 vSphere 用戶端設定 VMkernel 適配器，以實現可靠的連線和多路徑。

使用具有 vCenter Single Sign-On 的 vSphere 用戶端在 VI 工作負載域叢集上執行這些步驟。同一個 vSphere 用戶端同時管理管理域和工作負載域。
...
使用 vSphere 用戶端在工作負載域中的每個 ESXi 主機上執行下列步驟。
...
在工作負載域中的所有 ESXi 主機上設定 iSCSI 網路後，link:vmw-vcf-viwld-supp-iscsi-vvols-storage.html["為 iSCSI vVols配置存儲"] 。

...
---
= 使用 Trident Protect 保護 Red Hat OpenShift Virtualization 中的虛擬機
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
OpenShift Virtualization VM 的持續磁碟可由使用 link:https://docs.netapp.com/us-en/trident/["Trident犯罪現場調查"] 整合至 OpenShift 叢集的 ONTAP 儲存設備提供支援。在本節中、我們使用 link:https://docs.netapp.com/us-en/trident/trident-protect/learn-about-trident-protect.html["Trident Protect"] 建立 VM 的快照和備份、包括其資料磁碟區至 ONTAP 物件儲存設備。

然後，我們在需要時從快照或備份中還原。

Trident Protect 支援對 OpenShift 叢集上的應用程式和虛擬機器進行快照、備份、還原和災難復原。對於 OpenShift Virtualization 虛擬機器，Trident Protect 可保護的資料包括與虛擬機器關聯的 Kubernetes 資源物件、持久磁碟區和內部鏡像。
...
[%collapsible]
====
在為應用程式或虛擬機器建立快照和備份之前，必須在 Trident Protect 中配置物件儲存來儲存快照和備份。這可以透過儲存桶 CR 來實現。只有管理員才能建立和配置儲存桶 CR。在 Trident Protect 中，儲存桶 CR 稱為 AppVault。AppVault 物件是儲存桶的聲明式 Kubernetes 工作流程表示。AppVault CR 包含儲存桶在保護作業（例如備份、快照、復原作業和 SnapMirror 複製）中所需的設定。
...
這種單一檔案復原方法僅適用於在客戶端 pod 中掛載為檔案系統的 NAS/檔案型磁碟區，例如 NFS 磁碟區。它不適用於 SAN/原始區塊磁碟區，包括如前所示以 `volumeMode: Block`佈建的 VM 磁碟。

對於受支援的 NAS/檔案型磁碟區，還原特定檔案的簡單方法是讓用戶端可以存取後端磁碟區上的快照目錄。這可以透過編輯 Trident 後端配置來實現，新增 defaults 部分並將 `snapshotDir`參數設為 `true`。這樣就可以從與主磁碟區相同的掛載位置存取 `.snapshot`目錄。 `.snapshot`目錄包含所有快照。您可以切換到所需的快照目錄，找到要還原的檔案，然後將其複製到目標位置。


NOTE: 此方法會將歷程資料公開給具有掛載存取權限的工作負載/使用者，且可能會違反最小權限/合規性預期。請考量安全性/營運影響，並注意您應該暫時啟用此功能，並在還原檔案後將其還原。此外，如果您使用的是 NFS 版本 4，則在使用  `ls` 指令列出檔案時，將無法看到  `.snapshot` 目錄。不過，您可以  `cd` 到  `.snapshot` 目錄。

.以下範例展示如何透過存取 .snapshot 目錄並從快照複製檔案來還原單一檔案。
[%collapsible]
====
[source, yaml]
----
...
----
image:rh-os-n-use-case-ocpv-tp-dp-sfr-001.png["pod 和虛擬機器的 PVC"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-002.png["虛擬機器磁碟的 ONTAP 磁碟區名稱"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-003.png["用戶端中可見的快照目錄"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-004.png["虛擬機器中的掛載目錄"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-005.png["虛擬機器中遺失了一個檔案，需要將該檔案從快照目錄複製到主目錄進行還原"]

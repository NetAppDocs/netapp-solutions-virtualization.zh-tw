...
Shift 工具包可在 Windows 2019、2022 和 2025 版本上運作。從此處下載 Shift 工具包link:https://mysupport.netapp.com/site/tools/tool-eula/vm-data-collector["NetApp工具箱"]然後解壓縮軟體包，執行 exe 檔案進行安裝並啟動服務。 Shift 工具包提供 2 個軟體包－線上版（大小約 130MB）和離線版（大小約 1.2GB）。顧名思義，線上安裝套件會從網路下載並安裝必要的先決條件。
...
提示：請使用合適的部署軟體包。使用離線模式時，無需修改代理設置，因為所有必需的先決條件和檔案都已捆綁。
...
注意：為了確保 SVM 符合 Shift 工具包的要求，同時避免對生產 SVM 進行可能造成乾擾的修改，建立新的 SVM 是一種良好的做法。

注意：對於 VMware 到 Hyper-V 之間的雙向遷移，新的 SVM 應同時啟用 NFS 和 SMB 協定。所配置的磁碟區也應啟用 NFS 和 SMB。
...
注意：目標路徑必須與來源虛擬機器位於同一磁碟區上。

注意：根據目標虛擬機器管理程式類型和磁碟格式，為 qtree 指派正確的安全樣式。

注意：如果轉換後的 qcow2 檔案打算與 OpenShift 虛擬化一起使用，則有一個例外：這些檔案可以直接放置在磁碟區上，而無需使用 qtree。為了實現這一點，可以使用 Shift 工具包 GUI 或使用 Shift 工具包 API 將 VMDK 檔案轉換為 qcow2 格式，並將它們直接放置在磁碟區中。
...
注意：必須啟用 SMB 3.0，預設已啟用。

注意：請確保已啟用持續可用屬性。

注意：必須在儲存虛擬機器 (SVM) 上停用 SMB 匯出原則。

注意：CIFS 伺服器和 Hyper-V 伺服器所屬的網域必須同時允許 Kerberos 和 NTLMv2 驗證。

注意： ONTAP建立的共用具有 Windows 預設的共用權限，即「Everyone / 完全控制」。
...
注意：不支援 Red Hat Enterprise Linux 5 和 6 的 CentOS Linux/RedHat。

注意：不支援 Windows Server 2008，但轉換過程應該可以正常運作。請自行承擔風險；但是，我們收到了成功使用 Shift 工具包轉換 Windows 2008 VM 的客戶報告。遷移後更新 IP 位址非常重要，因為用於自動指派 IP 的 PowerShell 版本與 Windows Server 2008 上執行的舊版本不相容。
...
注意：在目前版本中，僅 Hyper-V、VMware、OpenShift 和 Oracle Virtualization 支援端對端虛擬機器遷移。

注意：在目前版本中，對於 KVM 作為目標，VMDK 到 qcow2 或 RAW 轉換是唯一支援的工作流程。因此，如果從下拉式選單中選擇了 KVM，則不需要虛擬機器管理程式詳細資訊。qcow2 磁碟可用於在 KVM 變體上設定虛擬機器。



== 安裝

. 下載link:https://mysupport.netapp.com/site/tools/tool-eula/vm-data-collector["Shift 工具包"]打包並解壓縮。
+
...
+
注意：所有預檢查均已執行，如果未滿足最低要求，則會顯示相應的錯誤或警告訊息。
...
+
注意：如果虛擬機器分配了多個網路卡，則設定過程中可以使用下拉選項選擇正確的 IP 位址。
...
+
注意：此憑證可以輕易地被第三方或 CA 產生的憑證取代。只需替換 certs 資料夾中的憑證即可。  certs 資料夾位於 <安裝目錄>Storage>Certs 中。
...
注意：如果 Shift 工具包 VM 沒有網路連接，離線安裝程式將執行相同的步驟，但會使用執行檔中包含的軟體包來安裝元件。
...
注意：對於沒有網路連線的部署，請從NetApp Toolchest 手動下載以「update」開頭的升級套件，並將其放置在指定資料夾（C:\NetApp_Shift）中。此資料夾路徑應在更新過程中建立並指定。其他步驟與線上升級流程相同。

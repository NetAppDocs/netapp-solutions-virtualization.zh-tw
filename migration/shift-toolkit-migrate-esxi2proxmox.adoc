...
* 對於 Windows 虛擬機，請確保 VirtIO ISO 映像已掛載；否則，準備程序將會失敗。VirtIO ISO 驅動程式可從link:https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/archive-virtio/virtio-win-0.1.271-1/virtio-win-0.1.271.iso["這裡"]下載。腳本將自動偵測已掛載的磁碟機並複製所需檔案。
+
** 請確保使用連結中指定的 ISO 文件，因為準備腳本會使用 .msi 軟體包來安裝驅動程式和 qemu-guest-agents。完成所有必要步驟後，登入 Shift toolkit UI，並將網站設定為以 Proxmox VE 為目標虛擬機器管理程式。若要新增站點，請按一下「Add New Site」並選擇「Destination」。
...
image::shift-toolkit-469.png[VM 磁碟映像已上傳至 Proxmox VE NFS 儲存池]

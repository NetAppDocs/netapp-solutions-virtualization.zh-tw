...
在計畫的維護窗口期內，虛擬機器必須正常關機。在開始遷移之前，請確認所有先決條件已滿足：

* 確保 Shift Toolkit VM 屬於網域。
* 確保 CIFS 共享配置了適當的權限
* 驗證用於遷移或轉換的 qtree 是否具有正確的安全樣式
* 使用叢集中任意 Hyper-V 主機上的 Hyper-V 管理器測試建立虛擬機，並將 VHDx 檔案放置在 CIFS 共用上。
* 使用「程式和功能」或 PowerShell 新增 Hyper-V 管理工具，從 Shift Toolkit VM 測試建立 VM： `add-windowsfeature rsat-hyper-v-tools`

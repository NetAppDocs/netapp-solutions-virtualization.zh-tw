...
* link:shift-toolkit-migrate-esxi2osvmtv.html["使用虛擬化遷移工具包 (MTV) 將 VMware ESXi 遷移到 Red Hat OpenShift Virtualization"]
* link:shift-toolkit-migrate-esxi2proxmox.html["VMware ESXi 到 Proxmox VE"]
...
從未執行過的藍圖可以直接從使用者介面刪除。但是，藍圖一旦被觸發，就會產生作業歷史記錄，必須先清除這些記錄才能刪除藍圖本身。使用 PowerShell 刪除指令碼清理這些作業記錄，之後即可刪除藍圖及其關聯的資源群組。

.步驟
. 從 Shift Toolkit UI 下載移除腳本，方法是導覽至 *Settings > Developer Access > Script block*。
+
image::shift-toolkit-082.png[根據藍圖名稱刪除作業]

. 解壓縮該壓縮包。壓縮包內包含兩個檔案：
+
** `removeBpJobs.json` - 設定檔案
** `removeBpJobs.ps1` - PowerShell 腳本


. 編輯 `removeBpJobs.json`檔案以指定：
+
** 此操作的執行名稱
** Shift Toolkit IP 位址
** 管理員憑證
...
. 開啟 PowerShell 並瀏覽至包含已解壓縮檔案的資料夾。
. 執行指令碼：
+
[listing]
----
...
腳本成功完成後，您可以從 Shift Toolkit UI 中刪除藍圖及其關聯的資源群組。

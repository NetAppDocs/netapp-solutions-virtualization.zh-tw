...
注意：Shift 工具包僅支援 qcow2 格式的磁碟轉換。它不支援虛擬機器創建或註冊。若要使用轉換後的磁碟，請手動建立虛擬機器並附加磁碟。
...
注意：要在虛擬機中使用轉換後的 VHDX 磁碟，必須透過 Hyper-V 管理器或 PowerShell 命令手動建立虛擬機，並將磁碟附加到該虛擬機。除此之外，還應手動映射網路。
...
+
注意：對於 OLVM，也可以使用 qcow2 檔案格式。此項目選擇可在建立藍圖時進行。

+
image::shift-toolkit-175.png[編輯轉換計劃步驟的對話框]

+
注意：要在虛擬機器中使用轉換後的 RAW 磁碟映像，必須透過 OpenShift 控制台或 OC 命令手動建立虛擬機器（透過 YAML 為 OpenShift 建立），或使用 OLVM REST API/UI 為 OLVM 創建，並且必須將磁碟附加到虛擬機器。除此之外，還應手動映射網路。

+
注意：手動建立虛擬機器時，請確保指定正確的啟動類型（EFI 或 BIOS）。
